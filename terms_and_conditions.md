# Terms and Conditions for Football Quiz

Last updated: April 15, 2026

These Terms and Conditions ("Terms") govern your use of the Football Quiz mobile application developed and operated by Artur Aleksandryan ("we", "us", "our", or "Developer").

By downloading, installing, accessing, or using Football Quiz, you agree to these Terms. If you do not agree, please do not use the app.

## 1. The App

Football Quiz is a free quiz game about football. The app provides football-related questions, categories, scores, progress tracking, optional Google Sign-In, cloud progress synchronization, and advertising-supported gameplay.

We may update, modify, suspend, or discontinue any part of the app at any time.

## 2. Eligibility

You must be at least 13 years old or the minimum age required by applicable law in your country to use the app. If you are under the age of majority, you should use the app only with permission from a parent or legal guardian.

## 3. Accounts and Google Sign-In

You can use parts of Football Quiz without signing in.

If you choose to sign in with Google, the app uses Firebase Authentication to identify your app account and synchronize your game progress. You are responsible for maintaining access to your Google account and for keeping your device secure.

We are not responsible for unauthorized access caused by your failure to secure your device, Google account, or authentication credentials.

## 4. Game Progress and Cloud Sync

Football Quiz may store progress locally on your device. If you sign in, the app may upload and restore your game progress using Firebase Cloud Firestore.

Cloud sync is provided for convenience. We try to keep it reliable, but we cannot guarantee that progress synchronization will always be uninterrupted, error-free, or available on every device or network.

## 5. Acceptable Use

You agree not to:

- Copy, modify, reverse engineer, decompile, or attempt to extract the source code of the app, except where permitted by applicable law.
- Use the app for unlawful, harmful, fraudulent, or abusive purposes.
- Interfere with the app, its servers, Firebase services, advertising systems, or other users.
- Attempt to gain unauthorized access to any account, system, database, or network.
- Use automated tools, scripts, or bots to manipulate gameplay, ads, analytics, or app behavior.

## 6. Intellectual Property

The app, including its design, code, structure, branding, and original content, is owned by Artur Aleksandryan or licensed to us and is protected by applicable intellectual property laws.

Football club names, logos, player names, competition names, and related references may be trademarks or intellectual property of their respective owners. Football Quiz is an independent quiz app and is not affiliated with, endorsed by, or sponsored by any football club, league, player, federation, or competition unless explicitly stated.

## 7. Questions and Accuracy

We work to keep quiz questions accurate and up to date. However, football records, squads, statistics, competitions, and historical interpretations can change or be corrected over time.

We do not guarantee that all questions, answers, statistics, or explanations will always be complete, current, or error-free.

## 8. Advertising and Third-Party Services

Football Quiz may display ads through Google AdMob and may use Google/Firebase services for authentication, analytics, messaging, and cloud data.

Third-party services are governed by their own terms and privacy policies. We are not responsible for third-party services, content, policies, or practices.

## 9. Internet Connection and Device Charges

Some features require an internet connection, including signing in, cloud sync, fetching questions, analytics, messaging, and ads.

You are responsible for any data charges, roaming charges, carrier fees, or other costs that may apply when using the app.

## 10. Privacy

Your use of the app is also governed by our Privacy Policy. Please read it carefully to understand how information is collected, used, stored, and deleted.

Privacy Policy: https://github.com/Arthur11232/Football-Quiz-Web/blob/main/privacy_policy.md

## 11. Data Deletion

If you are signed in, you can request deletion of your Football Quiz account data and cloud progress from the Profile screen inside the app.

The in-app deletion option removes the Firestore progress data associated with your app account, removes local game progress and locally stored custom profile photo data from the app, signs you out, and attempts to delete the Firebase Authentication app account record. In some cases, Firebase Authentication may require a recent sign-in before the app account record can be deleted.

You may also request deletion by contacting:

arthuralexandryan@gmail.com

Please include the email address used to sign in to the app so we can locate the correct account.

If you request deletion by email, we can delete the app data we control in cloud services, but we cannot remotely delete data that exists only on your device. You can remove local data by using the in-app deletion option, deleting the app, or clearing app data in your device settings.

## 12. No Warranty

Football Quiz is provided "as is" and "as available" without warranties of any kind, whether express or implied.

We do not guarantee that the app will always be available, secure, uninterrupted, error-free, compatible with every device, or free from defects.

## 13. Limitation of Liability

To the maximum extent permitted by applicable law, Artur Aleksandryan will not be liable for any indirect, incidental, special, consequential, or punitive damages, or for any loss of data, progress, profits, revenue, or goodwill arising from your use of or inability to use the app.

Nothing in these Terms limits rights that cannot be limited under applicable consumer protection laws.

## 14. Updates and Changes

We may update the app from time to time. Some features may require you to install the latest version.

We may also update these Terms. When we update them, we will change the "Last updated" date above. Continued use of the app after changes means that you accept the updated Terms.

## 15. Termination

We may suspend or terminate access to the app if you violate these Terms, misuse the app, interfere with services, or create legal or security risks.

You may stop using the app at any time by uninstalling it.

## 16. Contact Us

If you have questions or suggestions about these Terms, contact us at:

arthuralexandryan@gmail.com
